...
class NetworkService {
  ...
  Future<NetworkResponseModel> makeRequest(String url,
      {body, int timeOutInSec = 15, required RequestType type}) async {
    ...
    try {
      switch (type) {
        ...
        case RequestType.post:
          return http
              .post(Uri.parse(url), body: body, headers: _httpHeader)
              .timeout(Duration(seconds: timeOutInSec))
              .catchError(
                  (error) => http.Response('{"Status": "UnKnown"}', 500))
              .then((response) => parseResponse(response));
      }
    } catch (e) {
      return NetworkResponseModel(

          message: "LocaleKeys.sthWentWrong.locale");
    }
  }

  FutureOr<NetworkResponseModel> parseResponse(http.Response response) async {
    try {
      ...
      if (result["statusCode"] == 200) {
        ...
        return NetworkResponseModel(
            isSuccessful: true, data: data, message: result["message"]);
      }
      else {
        return NetworkResponseModel(
            isSuccessful: false,
            message:
                result["message"] ?? "LocaleKeys.sthWentWrong.locale");
      }
    } catch (e) {
      return NetworkResponseModel(
          message: "LocaleKeys.sthWentWrong.locale");
